...
public class TurmaDAO implements Map<String, Turma> {
    ...
    private TurmaDAO() {
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
            ...
            sql = "CREATE TABLE IF NOT EXISTS alunos (" +
                    ...
                    "Turma varchar(10), CONSTRAINT foreign key(Turma) references turmas(Id))";
            stm.executeUpdate(sql);
    ...
    public void clear() {
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             Statement stm = conn.createStatement()) {
            stm.executeUpdate("UPDATE alunos SET Turma=NULL");//vai à tabela dos alunos e coloca a fk(turma) de cada aluno a null
            stm.executeUpdate("TRUNCATE turmas");//elimina os dados da tabela turmas
        } catch (SQLException e) {
            // Database error!
            e.printStackTrace();
            throw new NullPointerException(e.getMessage());
    ...
    public Set<String> keySet() {
        ...
        try(Connection con = DriverManager.getConnection(DAOconfig.URL,DAOconfig.USERNAME,DAOconfig.PASSWORD);
            ...
            ResultSet rs = st.executeQuery("SELECT id FROM turmas")) {
                while(rs.next()){
                    turmas.add(rs.getString("id"));
                }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public Set<Map.Entry<String, Turma>> entrySet() {
        Map.Entry<String,Turma> entry;
        HashSet<Map.Entry<String, Turma>> col = new HashSet<>();
        try (Connection conn = DriverManager.getConnection(DAOconfig.URL, DAOconfig.USERNAME, DAOconfig.PASSWORD);
             Statement stm = conn.createStatement();
             ResultSet rs = stm.executeQuery("SELECT id FROM turmas")) {
            while (rs.next()) {
                entry =  new AbstractMap.SimpleEntry<>(rs.getString("id"),this.get(rs.getString("id")));
                col.add(entry);
            }
        } catch (Exception e) {
            e.printStackTrace();
